...
class Pagin extends StatefulWidget {
  @override
  _PaginState createState() => _PaginState();
}
...
class _ListPageState extends State<ListPage> {
  ...
  Future getPosts() async
  {
    ...
    QuerySnapshot qn= await firestore.collection("School").document("69").collection("Teachers").getDocuments();
    return qn.documents;
  }
...
class MapScreenState extends State<ProfilePage>
 ...
 TextEditingController controllerq;
  ...
  controllerq = TextEditingController(text: widget.post.data["qualification"]);
                              ...
                              child: new Row(
                                ...
                                children: <Widget>[
                                  ...
                                  new Column(
                                    ...
                                    mainAxisSize: MainAxisSize.min,
                                    children: <Widget>[
                                      _status ? _getEditIcon(context,rt2) : new Container(),
                                    ],
                                  )
                                ],
                              )),
...
child: Column(children: <Widget>[
  Row(children: <Widget>[
    Text("EDIT TEXT",style: TextStyle(fontWeight: FontWeight.bold,fontSize: 15),),
    Spacer(),
    IconButton(
...
Row(children: <Widget>[
  ...
  print('$name is'); 
  widget.post.data["first name"]=namecontroller1.text;
  widget.post.data["email"]=namecontroller.text;
  widget.post.data["mobile"]=namecontroller2.text;
  widget.post.data["qualification"]=namecontrollerq.text;
  widget.post.data["address"]=namecontrollera.text;
  widget.post.data["gender"]=namecontrollerg.text;
  widget.post.data["designation"]=namecontrollerd.text;
  updateuserdata( widget.post.data["mobile"]); });
//sprint(name); 
  },
  child: Text('Save Data'.toUpperCase(),style: TextStyle(color: Colors.white),), 
    )
    ),
],
),
...
final CollectionReference brew=Firestore.instance.collection('School').document('69').collection('Teachers');
