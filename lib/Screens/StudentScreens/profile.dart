...
class Profile1 extends StatefulWidget 
{
  ...
  Profile1(this.schoolCode,this.studentId);
  @override
  //final DocumentSnapshot post;
//  Profile({this.post,this.schoolCode});
  MapScreenState createState() => MapScreenState(schoolCode,studentId);
}

class MapScreenState extends State<Profile1>with SingleTickerProviderStateMixin 
{ 
  String schoolCode,studentId;
  MapScreenState(this.schoolCode,this.studentId);
  ...
  final namecontrollers  = new TextEditingController();
  
  final namecontrollerc  = new TextEditingController();
...
var val=Firestore.instance.collection('School').document(schoolCode).
      collection("Student").document(studentId).get().then((value){
    //if(value.data['url'])
    link=value.data['url'];
    if(link==null)
    link='#';
 ...
 Future<String>getD() async
 {
       ...
       .ref()    
       .child('student/${namecontroller2.text}');    
  ...
  void initState() {
   ...
   check();
    ...
    controllerq = TextEditingController(text: datas.data['father \'s name']);
    namecontrollera.text = datas.data['mother \'s name'];
    controllera = TextEditingController(text: datas.data['mother \'s name']);
...
//   bool isWeb = UniversalPlatform.isWeb;
  Future<void> _addPathToDatabase(String text) async {
    try {
      ...
      collection("Student").document(studentId).setData({'url':imageString , 'location':text},merge: true);
    }catch(e){
      print(e.message);
      showDialog(
          context: context,
...
/*Future uploadPicWeb(BuildContext context) async{
  ...
  Future uploadPic(BuildContext context) async {    
   ...
   //    getUrl();
   _addPathToDatabase("student/${namecontroller2.text}");
...
*/
                  ...
                  Row(
                    children: <Widget>[
                      Expanded(
                        ...
                        decoration: InputDecoration(hintText: 'Father \'s Name'),
                        // controller: controller4,
                        //  autofocus: true,
                      )),
                    ],
                  ...
                  Row(
                    children: <Widget>[
                      Expanded(
                        ...
                        decoration: InputDecoration(hintText: 'Mother \'s Name'),
                        // controller: controller4,
                        //  autofocus: true,
                      )),
                    ],
                      ...
                      Expanded(
                        ...
                        onPressed: () {
                          setState(() {
                            ...
                            controllers.text = namecontrollers.text;
                            controllerc.text = namecontrollerc.text;
  ...
  DocumentReference dc=Firestore.instance.collection('School').document(schoolCode).
    ...
    datas.data['mobile']=namecontroller2.text;
                            ...
                            datas.data["father \'s name"] = namecontrollerq.text;
                        datas.data["mother \'s name"] = namecontrollera.text;
...
//sprint(name);
  ...
  Future updateuserdata(String rt2) async {
    ...
    return await brew.document(rt2).setData({
      ...
      'mother \'s name': namecontrollera.text,
      'gender': namecontrollerg.text,
      'rollno': namecontrollerd.text,
